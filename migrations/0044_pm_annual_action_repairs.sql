...
-- Every active queue item now owns a real repair so parts can be planned before
...
-- Convert existing pending/attached text-only Next PM items into real repairs.
INSERT INTO repairs (
...
WHERE n.repair_id IS NULL
  AND n.status IN ('pending', 'attached');
...
-- Existing open repairs on a unit become required work on any PM/Annual that is
-- already open when this migration lands.
...
SELECT
  ...
  CURRENT_TIMESTAMP,
  child.id,
  CASE WHEN target.source = 'scheduled-annual' THEN 'annual' ELSE 'pm' END
FROM repairs target
JOIN repairs child
  ON child.equipment_id = target.equipment_id
 AND child.id <> target.id
WHERE target.source IN ('scheduled-pm', 'scheduled-annual')
...
-- When a future scheduled PM/Annual is created, activate matching deferred repairs
-- and attach all other already-open repairs on that unit to the maintenance visit.
CREATE TRIGGER trg_attach_maintenance_actions_to_new_visit
...
BEGIN
  ...
  WHERE equipment_id = NEW.equipment_id
    ...
    AND target_event_type = CASE WHEN NEW.source = 'scheduled-annual' THEN 'annual' ELSE 'pm' END;
  ...
  SELECT
    ...
    CURRENT_TIMESTAMP,
    r.id,
    CASE WHEN NEW.source = 'scheduled-annual' THEN 'annual' ELSE 'pm' END
  FROM repairs r
...
-- A normal repair created while a PM/Annual is already open becomes required on
-- that maintenance visit. Deferred repairs are intentionally excluded.
CREATE TRIGGER trg_attach_new_repair_to_open_maintenance
AFTER INSERT ON repairs
...
BEGIN
  ...
  WHERE target.equipment_id = NEW.equipment_id
    AND target.source IN ('scheduled-pm', 'scheduled-annual')
    AND lower(COALESCE(target.status, '')) NOT LIKE '%complete%'
    AND NOT EXISTS (
      SELECT 1
      FROM pm_next_repairs n
...
-- Old Next-PM controls are not allowed to defer or administratively complete a
-- real repair once that repair is attached to a current maintenance visit.
...
-- The inspection itself cannot be marked ready while any attached real repair is open.
...
BEGIN
  SELECT CASE WHEN EXISTS (
    ...
    WHERE n.target_repair_id = NEW.repair_id
      ...
      AND lower(COALESCE(r.status, '')) NOT LIKE '%complete%'
  ) THEN RAISE(ABORT, 'Complete all repairs attached to this PM/Annual before finishing the inspection.') END;
END;
...
BEGIN
  SELECT CASE WHEN EXISTS (
    ...
    WHERE n.target_repair_id = NEW.id
      AND n.status <> 'cancelled'
      AND lower(COALESCE(r.status, '')) NOT LIKE '%complete%'
  ) THEN RAISE(ABORT, 'Complete all repairs attached to this PM/Annual before closing the work order.') END;
END;
