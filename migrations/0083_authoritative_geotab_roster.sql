...
SELECT 'semi', CAST(n AS TEXT) || '(' || suffix || ')', n
FROM nums;
...
SELECT
  'trailer',
  CASE WHEN n < 1000 THEN 'TRL ' || n ELSE CAST(n AS TEXT) END,
  n
FROM nums;

-- Reactivate an existing semi on the master list only when another active copy
-- does not already exist. The two known Geotab name drifts are preserved here
-- without renaming so historical duplicate rows are not forced together blindly.
UPDATE equipment
SET active = 1,
    archived_at = NULL,
    archive_reason = NULL,
...
-- Three-digit GPS trailers are canonical as "TRL 202"; both TRL202 and TRL 202
-- existing spellings are accepted. Five-digit GPS trailers use the bare number.
UPDATE equipment
SET active = 1,
    archived_at = NULL,
    archive_reason = NULL,
...
-- Create only master-list units that are genuinely absent. They remain without
-- a telemetry assignment until a Geotab device is explicitly selected.
...
-- Archive semi-style equipment that is outside the approved semi list. Company
-- cars/vans/switchers do not use these parenthesized fleet suffixes and are left
-- unchanged.
UPDATE equipment
SET active = 0,
...
-- Archive only GPS/Geotab-linked trailers outside the supplied tracked trailer
-- roster. Manual trailers that are not Geotab-linked are intentionally retained.
UPDATE equipment
SET active = 0,
    archived_at = COALESCE(archived_at, CURRENT_TIMESTAMP),
...
-- Archived/inactive equipment cannot remain in the current telemetry roster.
...
WHERE current = 1
  AND equipment_id IN (
    SELECT id
    FROM equipment
    WHERE active = 0 OR archived_at IS NOT NULL
  );

-- Backfill explicit current assignments from trusted device IDs for active master
-- semis/trailers and company vehicles. From this point, assignment is the only
-- production mileage link; name/VIN matching no longer grants tracking authority.
...
SELECT
  e.id, e.geotab_device_id, e.unit, e.vin,
...
WHERE e.active = 1
  ...
  AND NOT EXISTS (
    ...
    FROM equipment_geotab_devices a
    WHERE a.current = 1
      AND (a.equipment_id = e.id OR a.geotab_device_id = e.geotab_device_id)
  );
