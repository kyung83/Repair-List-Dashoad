...
export default function RepairTypesPage(){
 ...
 return <main style={page}>
  <header style={header}><div><p style={eyebrow}>SETUP</p><h1 style={title}>Repair Types</h1><p style={sub}>These are the master repair categories for work orders, review, and reporting. You can add, rename, reorder, disable, or give a category its own check sheet.</p></div><button style={primary} onClick={()=>void addType()} disabled={busy}>+ ADD REPAIR TYPE</button></header>
  ...
  <div style={layout}>
   ...
   <section style={panel}>{selected?<>
    ...
    {name.toUpperCase()==="INDIRECT LABOR-OTHER"&&<div style={info}><strong>Shop / No Unit labor</strong><span>This category is reserved for labor that does not belong to a truck or trailer. The no-unit mechanic workflow is being enabled separately in Part 3.</span></div>}
