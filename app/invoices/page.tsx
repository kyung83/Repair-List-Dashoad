...
const input={padding:"10px 11px",border:"1px solid #cbd5e1",borderRadius:8,background:"white",boxSizing:"border-box" as const};
...
export default function InvoicesPage(){
 ...
 const[form,setForm]=useState({workOrderId:"",customerId:"",billToName:"",invoiceDate:new Date().toISOString().slice(0,10),dueDate:"",taxRate:"0",extraDescription:"",extraAmount:"",notes:""});
 ...
 async function createInvoice(e:FormEvent){
  ...
  const result=await post({
   ...
   invoiceDate:form.invoiceDate,dueDate:form.dueDate,taxRate:Number(form.taxRate),extraDescription:form.extraDescription,
   extraAmount:Number(form.extraAmount||0),notes:form.notes,
  });
  ...
  <form onSubmit={createInvoice} style={{...panel,marginTop:18}}>
   ...
   <div style={{display:'grid',gridTemplateColumns:'minmax(300px,390px) minmax(620px,1fr)',gap:14,marginTop:14,alignItems:'start'}}>
    <aside style={{border:'1px solid #d9e0e5',borderRadius:10,overflow:'hidden',background:'#fafbfc'}}>
     <div style={{padding:10,borderBottom:'1px solid #d9e0e5'}}><input value={workOrderSearch} onChange={e=>setWorkOrderSearch(e.target.value)} placeholder="Search unit, repair, technician, part..." style={{...input,width:'100%'}}/><div style={{fontSize:10,color:'#6c7884',marginTop:6}}>{visibleWorkOrders.length}{workOrders.length>150&&!workOrderSearch?' of ': ' '}completed work order{workOrders.length===1?'':'s'} shown</div></div>
...
function PreviewSection({title,children}:{title:string;children:React.ReactNode}){return <section style={{borderTop:'1px solid #dbe2e7'}}><div style={{padding:'6px 9px',fontSize:10,fontWeight:900,letterSpacing:'.05em',textTransform:'uppercase',background:'#eef2f4',color:'#334555'}}>{title}</div>{children}</section>;}
