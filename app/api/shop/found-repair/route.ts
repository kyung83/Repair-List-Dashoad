...
export async function POST(request: Request) {
  try {
    ...
    const result = await env.DB.prepare(`
      ...
      VALUES (?, ?, 'Open', '2', 'manual', ?, ?, NULL, CURRENT_TIMESTAMP)
    ...
    `).bind(id, user.id, technician.id, `${technician.name} found additional work on Unit ${active.unit || active.equipment_id}: ${issue}. Repair Type will be selected when the repair is worked.`.slice(0, 500)).run();
