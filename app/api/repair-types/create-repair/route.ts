...
export async function POST(request:Request){
 try{
  ...
  const body=await request.json() as Record<string,unknown>;const repairType=await requireRepairType(env.DB,body.repairTypeId);const issue=String(body.issue??'').trim().slice(0,500),parts=String(body.parts??'').trim().slice(0,1000),priority=Number(body.priority??2);if(!issue)throw new Error('Enter the repair needed.');if(![1,2,3].includes(priority))throw new Error('Priority must be 1, 2, or 3.');
  ...
  const result=await env.DB.prepare(`INSERT INTO repairs(equipment_id,title,parts_text,status,priority,source,location,technician_id,repair_type_id,updated_at) VALUES(?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)`).bind(equipmentId,issue,parts,status,String(priority),source,equipmentId===null?'':location,technician?.id??null,repairType.id).run();const repairId=Number(result.meta.last_row_id);if(!repairId)throw new Error('Repair could not be added.');
  await env.DB.prepare(`INSERT INTO repair_job_events(repair_id,user_id,technician_id,action,detail) VALUES(?,?,?,'repair_created',?)`).bind(repairId,user.id,technician?.id??null,`${user.displayName} created ${repairType.name}: ${issue}${equipmentId===null?' (SHOP / NO UNIT)':` for Unit ${unit}`}${technician?` and assigned it to ${technician.name}`:''}.`.slice(0,500)).run();
 ...
 }catch(error){console.error(JSON.stringify({event:'categorized_repair_create_failed',error:String(error)}));return Response.json({error:error instanceof Error?error.message:'Repair could not be added.'},{status:400})}
}
