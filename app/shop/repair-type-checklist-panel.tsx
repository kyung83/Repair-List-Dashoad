...
export default function RepairTypeChecklistPanel({repairId,canWork}:Props){
 ...
 async function post(body:Record<string,unknown>){setBusy(true);setMessage("");try{const response=await fetch("/api/repair-type-checklist",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...body,repairId})});const payload=await response.json() as Data&{ok?:boolean;error?:string};if(!response.ok||!payload.ok)throw new Error(payload.error||"Checklist change failed.");accept(payload);return payload}catch(error){setMessage(error instanceof Error?error.message:"Checklist change failed.");return null}finally{setBusy(false)}}
