...
function refreshReview(repairId:string){
  window.dispatchEvent(new CustomEvent("repair-review-refresh",{detail:{repairId}}));
}
...
export default function TechnicianRepairToolsV2({repairId,canWork}:Props){
  ...
  async function saveNote(){
    ...
    try{
      ...
      setNote("");setNoteMessage("Note saved. It is also in the Review Before Finishing section at the bottom.");
      await loadNotes();refreshReview(repairId);
  ...
  async function useOrRequestPart(){
    ...
    try{
      ...
      setSearch("");setSelectedPart(null);setQuantity(1);await loadParts();refreshReview(repairId);
  ...
  async function requestTypedPart(){
    ...
    try{
      ...
      setSearch("");setSelectedPart(null);setQuantity(1);refreshReview(repairId);
  ...
  return <section style={toolsPanel}>
    ...
    <div style={toolCard}>
      <div><strong style={heading}>PART LOOKUP</strong><span style={help}>Apply or request the part here. The running list of what was actually used is kept in Review Before Finishing at the bottom.</span></div>
