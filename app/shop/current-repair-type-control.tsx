...
type Data={ok?:boolean;error?:string;repairId:string;current:RepairType|null;types:RepairType[];maintenanceEventType:"pm"|"annual"|null;maintenanceLabel:string;noUnit:boolean;locked:boolean};
...
export default function CurrentRepairTypeControl({repairId}:Props){
  ...
  async function choose(value:string){
    ...
    }catch(error){setMessage(error instanceof Error?error.message:"Repair Type could not be saved.");await load()}
  ...
  return <section id="current-repair-type" style={data.current?box:requiredBox}>
    <div style={{display:"grid",gap:4}}><span style={label}>REPAIR TYPE {data.current?"":"· REQUIRED"}</span><strong style={headline}>{data.current?.name||"Choose what type of repair this is"}</strong><small style={help}>{data.current?"You can change it until a category checklist is started.":"Select this while you are working. Repair entry stays fast for the office."}</small></div>
    <select value={data.current?.id?String(data.current.id):""} disabled={busy} onChange={event=>void choose(event.target.value)} style={select}>
    ...
    {checklist&&<div style={checklistNote}>{data.current?.name} uses a check sheet. It will appear below after this selection.</div>}
