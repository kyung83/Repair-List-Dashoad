...
export default function TechnicianRepairTools({repairId,canWork}:Props){
  ...
  async function useOrRequestPart(){
    ...
    if(!Number.isFinite(quantity)||quantity<=0){setPartMessage("Enter a positive quantity.");return}
  ...
  return <section style={toolsPanel}>
    ...
    <div style={toolCard}>
      <div><strong style={heading}>PART LOOKUP</strong><span style={help}>Type any part number or description. Pick the match, quantity, then Use / Request.</span></div>
      <input value={search} onChange={event=>{setSearch(event.target.value);setSelectedPart(null);setPartMessage("")}} placeholder="Type part number or description…" style={input} disabled={partBusy||!canWork}/>
      {matches.length>0&&<div style={results}>{matches.map(part=><button key={part.id} type="button" onClick={()=>{setSelectedPart(part);setSearch(`${part.partNumber} — ${part.description}`)}} style={selectedPart?.id===part.id?selectedResult:resultButton}><span><strong>{part.partNumber}</strong> — {part.description}</span><span style={availability}>{qty(part.available??part.quantityOnHand)} available</span></button>)}</div>}
      {search.trim()&&matches.length===0&&!selectedPart&&<div style={messageStyle}>No matching part number or description.</div>}
      {selectedPart&&<div style={selectedBox}><div><strong>{selectedPart.partNumber}</strong><div style={small}>{selectedPart.description}</div></div><input aria-label="Part quantity" type="number" min="0.01" step="any" value={quantity} onChange={event=>setQuantity(Number(event.target.value))} style={qtyInput} disabled={partBusy}/><button type="button" onClick={()=>void useOrRequestPart()} style={partButton} disabled={partBusy}>{partBusy?"Saving…":"USE / REQUEST"}</button></div>}
...
const selectedBox={display:"grid",gridTemplateColumns:"minmax(160px,1fr) 90px auto",gap:8,alignItems:"center",padding:10,border:"1px solid #a8bfd6",borderRadius:9,background:"white"} as const;
const small={fontSize:11,color:"#667482",marginTop:2} as const;
const qtyInput={width:"100%",boxSizing:"border-box" as const,padding:"10px",border:"1px solid #ccd4db",borderRadius:8} as const;
const partButton={border:0,borderRadius:9,padding:"10px 12px",background:"#173a5d",color:"white",fontWeight:900,cursor:"pointer"} as const;
