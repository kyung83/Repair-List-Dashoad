...
export default function ShopPage(){
  ...
  return <main style={{minHeight:"100vh",background:"#f3f5f7",padding:"30px 30px 90px",color:"#182331"}}>
    ...
    {data?.activeTimer&&<section style={workingBanner}><div><p style={{margin:"0 0 5px",fontSize:12,fontWeight:900,letterSpacing:".16em",color:"#ff9a4c"}}>WORKING NOW</p><button onClick={()=>setSelectedId(data.activeTimer!.repairId)} style={activeJobButton}>Unit {data.activeTimer.unit||"—"} — {data.activeTimer.title}</button><div style={{marginTop:5,color:"#cbd6df",fontSize:12}}>Labor is tracking automatically while this unit is open.</div></div><span style={timerStyle}>{duration(data.activeTimer.startedAt,now)}</span></section>}
    ...
    {workspaceRepair&&data&&<section style={workspaceStyle}>
      <div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"center",flexWrap:"wrap"}}><div><p style={eyebrow}>{data.activeTimer?"CURRENT UNIT":"UNIT WORKSPACE"}</p><h2 style={{margin:"6px 0 4px",fontSize:28}}><a href={unitInfoHref(workspaceRepair.unit)} target="_blank" rel="noreferrer" style={unitInfoLink}>Unit {workspaceRepair.unit||"—"} ↗</a></h2><div style={{color:"#667482"}}>{unitRepairs.length} open repair{unitRepairs.length===1?"":"s"} · {prettyYard(workspaceRepair.yard)} inventory · click unit for VIN/mileage</div></div></div>
...
const doneWorkingButton={...outcomeBase,background:"#e8edf1",color:"#253440",border:"1px solid #cfd9e2"} as const;
