...
export default function ShopPage(){
  ...
  async function doneUnit(){const result=await action({action:"doneUnit"});if(result){setNeedPart(false);setHandoffOpen(false)}}
  ...
  return <main style={{minHeight:"100vh",background:"#f3f5f7",padding:"30px 30px 90px",color:"#182331"}}>
    ...
    {workspaceRepair&&data&&<section style={workspaceStyle}>
      <div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"center",flexWrap:"wrap"}}><div><p style={eyebrow}>UNIT WORKSPACE</p><h2 style={{margin:"6px 0 4px",fontSize:28}}><a href={unitInfoHref(workspaceRepair.unit)} target="_blank" rel="noreferrer" style={unitInfoLink}>Unit {workspaceRepair.unit||"—"} ↗</a></h2><div style={{color:"#667482"}}>{unitRepairs.length} open repair{unitRepairs.length===1?"":"s"} · {prettyYard(workspaceRepair.yard)} inventory · click unit for VIN/mileage</div></div>{data.activeTimer&&activeRepair&&sameUnit(activeRepair,workspaceRepair)&&<div style={unitSessionActions}><button disabled={busy} onClick={()=>setHandoffOpen(open=>!open)} style={handoffButton}>HAND OFF TO NEXT SHIFT</button><button disabled={busy} onClick={()=>void doneUnit()} style={doneUnitButton}>DONE WORKING ON UNIT</button></div>}</div>

      {handoffOpen&&data.activeTimer&&activeRepair&&sameUnit(activeRepair,workspaceRepair)&&<div style={handoffPanel}><div><strong style={{fontSize:16,color:"#17324a"}}>Hand unfinished work to the next shift</strong><div style={{marginTop:4,fontSize:12,color:"#60717e"}}>Your current labor will be saved. Parts used, requested parts, pictures, notes, DVIR links, and existing work stay on the same repair records.</div></div><label style={handoffLabel}>Hand off to<select value={handoffTechId} onChange={event=>setHandoffTechId(event.target.value)} style={inputStyle} disabled={busy}><option value="">Leave Unassigned — next shift can pick it up</option>{handoffTechnicians.map(tech=><option key={tech.id} value={tech.id}>{tech.name}</option>)}</select></label><label style={handoffLabel}>What is left to do? *<textarea aria-label="Shift handoff note" value={handoffNote} onChange={event=>setHandoffNote(event.target.value)} maxLength={500} rows={3} placeholder="Example: Left rear chamber replaced; still needs air leak check and road test." style={{...inputStyle,resize:"vertical"}} disabled={busy}/></label><div style={{display:"flex",gap:8,justifyContent:"flex-end",flexWrap:"wrap"}}><button type="button" disabled={busy} onClick={()=>{setHandoffOpen(false);setHandoffTechId("");setHandoffNote("")}} style={doneUnitButton}>CANCEL</button><button type="button" disabled={busy||!handoffNote.trim()} onClick={()=>void handoffUnit()} style={confirmHandoffButton}>CONFIRM HANDOFF</button></div></div>}
      ...
      {selected&&sameUnit(selected,workspaceRepair)&&(()=>{const running=data.activeTimer?.repairId===selected.id,repairMine=selected.technicianId===data.user.technicianId&&data.user.technicianId!==null,repairAvailable=selected.technicianId===null,canStart=!data.activeTimer&&Boolean(data.user.technicianId)&&(repairMine||repairAvailable);return <div style={{marginTop:20,paddingTop:18,borderTop:"1px solid #e0e5e9"}}>
        ...
        <div style={{marginTop:20,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(310px,1fr))",gap:16}}><div style={workspaceCard}><h3 style={workspaceHeading}>Parts</h3>
          ...
          {repairMine&&<div style={{marginTop:12,fontSize:12,color:"#667482"}}>Use <strong>PART LOOKUP</strong> above to apply inventory parts or request exactly what you need.</div>}
