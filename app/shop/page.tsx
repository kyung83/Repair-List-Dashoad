...
type Repair={id:string;equipmentId:number|null;unit:string;issue:string;status:string;location:string;technicianId:number|null;assignedTo:string;laborHours:number;plannedParts:PlannedPart[];usedParts:UsedPart[];laborEntries:LaborEntry[];yard?:string;handoffNote?:string;handoffAt?:string};
...
export default function ShopPage(){
  ...
  useEffect(()=>{const refresh=()=>void load().catch(error=>setMessage(error instanceof Error?error.message:"Shop jobs could not be refreshed."));window.addEventListener("shop-jobs-refresh",refresh);return()=>window.removeEventListener("shop-jobs-refresh",refresh)},[]);
  ...
  async function action(body:Record<string,unknown>){
    ...
    try{
      ...
      else if(result.handoff)setMessage(`Shift handoff saved. ${result.handedOffCount||1} open repair${result.handedOffCount===1?"":"s"} moved to ${result.handoffTarget||"the next shift"}. Your labor was saved and all parts/work stayed with the repair.`);
      ...
      else if(result.waitingOnPart)setMessage(result.nextRepairId?"Part request saved. Labor was saved, this repair moved to Waiting on Part, and the next repair started.":"Part request saved. Labor was saved and this repair moved to Waiting on Part.");
  ...
  async function doneUnit(){const result=await action({action:"doneUnit"});if(result){setNeedPart(false);setHandoffOpen(false);setHandoffTechId("");setHandoffNote("")}}
  async function handoffUnit(){const note=handoffNote.trim();if(!note){setMessage("Enter what is left to do for the next shift.");window.requestAnimationFrame(()=>document.querySelector<HTMLTextAreaElement>('[aria-label="Shift handoff note"]')?.focus());return}const result=await action({action:"doneUnit",handoff:true,targetTechnicianId:handoffTechId?Number(handoffTechId):null,handoffNote:note});if(result){setNeedPart(false);setHandoffOpen(false);setHandoffTechId("");setHandoffNote("")}}

  async function usePlannedPart(repair:Repair,planned:PlannedPart){const remaining=Math.max(0,planned.quantity-planned.usedQuantity);if(remaining<=0)return;const result=await action({action:"usePart",repairId:repair.id,partId:planned.partId,quantity:remaining});if(result)setMessage(result.awaitingParts?`${planned.partNumber}: Parts Desk updated automatically. Labor was saved and this repair moved to Waiting on Part.`:`${numberText(result.usedImmediately||remaining)} × ${planned.partNumber} used from ${result.warehouseCode}.`)}
  ...
  const handoffTechnicians=useMemo(()=>(data?.technicians??[]).filter(tech=>tech.id!==data?.user.technicianId),[data]);
  ...
  return <main style={{minHeight:"100vh",background:"#f3f5f7",padding:"30px 30px 90px",color:"#182331"}}>
    ...
    {!data?.activeTimer&&readyForMe.length>0&&<section style={readySection}><strong style={{color:"#176440"}}>Parts ready for {readyForMe.length} repair line{readyForMe.length===1?"":"s"}</strong><div style={{fontSize:12,color:"#5e7567",marginTop:3}}>Receiving allocated these automatically.</div><div style={{display:"flex",gap:7,flexWrap:"wrap",marginTop:9}}>{readyForMe.slice(0,6).map(request=><button key={request.id} onClick={()=>setSelectedId(request.repairId)} style={readyButton}>Unit {request.unit||"—"} · {request.partNumber} · {numberText(request.reservedQuantity)} ready</button>)}</div></section>}

    {data?.activeTimer&&<section style={workingBanner}><div><p style={{margin:"0 0 5px",fontSize:12,fontWeight:900,letterSpacing:".16em",color:"#ff9a4c"}}>WORKING NOW</p><button onClick={()=>setSelectedId(data.activeTimer!.repairId)} style={activeJobButton}>Unit {data.activeTimer.unit||"—"} — {data.activeTimer.title}</button><div style={{marginTop:5,color:"#cbd6df",fontSize:12}}>Focus mode is on. Other units stay hidden until this unit session is finished.</div></div><span style={timerStyle}>{duration(data.activeTimer.startedAt,now)}</span></section>}

    {!data?.activeTimer&&<section style={{marginTop:22,display:"flex",gap:10,flexWrap:"wrap"}}><button onClick={()=>setView("mine")} style={view==="mine"?activeTab:tabButton}>My Units ({mineGroups.length})</button><button onClick={()=>setView("available")} style={view==="available"?activeTab:tabButton}>Available Units ({availableGroups.length})</button><button onClick={()=>setView("all")} style={view==="all"?activeTab:tabButton}>All Open Units ({allGroups.length})</button></section>}

    {workspaceRepair&&data&&<section style={workspaceStyle}>
      <div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"center",flexWrap:"wrap"}}><div><p style={eyebrow}>UNIT WORKSPACE</p><h2 style={{margin:"6px 0 4px",fontSize:28}}><a href={unitInfoHref(workspaceRepair.unit)} target="_blank" rel="noreferrer" style={unitInfoLink}>Unit {workspaceRepair.unit||"—"} ↗</a></h2><div style={{color:"#667482"}}>{unitRepairs.length} open repair{unitRepairs.length===1?"":"s"} · {prettyYard(workspaceRepair.yard)} inventory · click unit for VIN/mileage</div></div>{data.activeTimer&&activeRepair&&sameUnit(activeRepair,workspaceRepair)&&<button disabled={busy} onClick={()=>setHandoffOpen(open=>!open)} style={doneUnitButton}>DONE WORKING ON UNIT</button>}</div>

      {handoffOpen&&data.activeTimer&&activeRepair&&sameUnit(activeRepair,workspaceRepair)&&<div style={handoffPanel}><div><strong style={{fontSize:16,color:"#17324a"}}>Done for now or hand off to the next shift?</strong><div style={{marginTop:4,fontSize:12,color:"#60717e"}}>Either choice saves your current labor. Nothing already recorded on the repair is lost.</div></div><button type="button" disabled={busy} onClick={()=>void doneUnit()} style={doneForNowButton}>DONE FOR NOW — KEEP ASSIGNED TO ME</button><div style={handoffDivider}>OR HAND OFF TO NEXT SHIFT</div><label style={handoffLabel}>Hand off to<select value={handoffTechId} onChange={event=>setHandoffTechId(event.target.value)} style={inputStyle} disabled={busy}><option value="">Leave Unassigned — next shift can pick it up</option>{handoffTechnicians.map(tech=><option key={tech.id} value={tech.id}>{tech.name}</option>)}</select></label><label style={handoffLabel}>What is left to do? *<textarea aria-label="Shift handoff note" value={handoffNote} onChange={event=>setHandoffNote(event.target.value)} maxLength={500} rows={3} placeholder="Example: Left rear chamber replaced; still needs air leak check and road test." style={{...inputStyle,resize:"vertical"}} disabled={busy}/></label><div style={{display:"flex",gap:8,justifyContent:"flex-end",flexWrap:"wrap"}}><button type="button" disabled={busy} onClick={()=>{setHandoffOpen(false);setHandoffTechId("");setHandoffNote("")}} style={doneUnitButton}>CANCEL</button><button type="button" disabled={busy} onClick={()=>void handoffUnit()} style={confirmHandoffButton}>HAND OFF TO NEXT SHIFT</button></div></div>}

      <div style={{marginTop:16,display:"grid",gap:9}}>{unitRepairs.map(repair=>{const state=lineState(repair,data.activeTimer?.repairId),rowMine=repair.technicianId===data.user.technicianId&&data.user.technicianId!==null,rowAvailable=repair.technicianId===null,clickable=rowMine||rowAvailable||repair.id===data.activeTimer?.repairId;return <button key={repair.id} disabled={busy||!clickable} onClick={()=>void chooseRepair(repair)} style={{...repairRow,...(state==="WORKING NOW"?workingRow:state==="WAITING ON PART"?waitingRow:{})}}><div style={{minWidth:0,textAlign:"left"}}><div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap"}}><strong style={{fontSize:17,color:"#172431"}}>{repair.issue}</strong>{repair.handoffNote&&<span style={handoffBadge}>SHIFT HANDOFF</span>}</div><span style={{display:"block",marginTop:3,fontSize:12,color:"#687783"}}>{rowAvailable?"Unassigned":`Assigned to ${repair.assignedTo||"technician"}`} · {repair.laborHours.toFixed(2)} hr logged</span></div><span style={state==="WORKING NOW"?workingBadge:state==="WAITING ON PART"?waitingBadge:openBadge}>{state}</span></button>})}</div>
      ...
      {selected&&sameUnit(selected,workspaceRepair)&&(()=>{const running=data.activeTimer?.repairId===selected.id,repairMine=selected.technicianId===data.user.technicianId&&data.user.technicianId!==null,repairAvailable=selected.technicianId===null,canStart=!data.activeTimer&&Boolean(data.user.technicianId)&&(repairMine||repairAvailable);return <div style={{marginTop:20,paddingTop:18,borderTop:"1px solid #e0e5e9"}}>
        ...
        {selected.handoffNote&&<div style={handoffNotice}><strong>SHIFT HANDOFF</strong><div style={{marginTop:4}}>{selected.handoffNote}</div></div>}

        {running&&<><div style={outcomeGrid}><button disabled={busy} onClick={()=>void reportOutcome("repaired",selected)} style={repairedButton}><strong>REPAIRED</strong><span style={outcomeHint}>Save labor · close this repair · continue</span></button><button disabled={busy} onClick={()=>void reportOutcome("skip",selected)} style={skipButton}><strong>SKIP FOR NOW</strong><span style={outcomeHint}>Save labor · leave open · continue</span></button><FoundRepairControl repairId={selected.id} unit={selected.unit} onAdded={load}/></div>
          {needPart&&<div style={partPrompt}><div><strong>Catalog part fallback</strong><div style={{fontSize:12,color:"#6c7882",marginTop:2}}>Use the typed Part Lookup above first. Use this only if you still need to select a known catalog part.</div></div><select value={partId} onChange={event=>setPartId(event.target.value)} style={inputStyle} disabled={busy}><option value="">Search / select part</option>{data.parts.map(part=><option key={part.id} value={part.id}>{part.partNumber} — {part.description} ({numberText(availabilityFor(part,selected))} available)</option>)}</select><input aria-label="Part quantity" type="number" min="0.01" step="any" value={partQuantity} onChange={event=>setPartQuantity(Number(event.target.value))} style={inputStyle} disabled={busy}/><button disabled={busy} onClick={()=>void submitWaitingPart(selected)} style={promptDoneButton}>DONE</button></div>}</>}
        ...
        <div style={{marginTop:20,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(310px,1fr))",gap:16}}><div style={workspaceCard}><h3 style={workspaceHeading}>Parts</h3>
          ...
          {repairMine&&<div style={{marginTop:12,fontSize:12,color:"#667482"}}>Use <strong>PART LOOKUP</strong> above. If stock is short, the request automatically saves labor and moves this repair to Waiting on Part.</div>}
    ...
    {!data?.activeTimer&&<section style={{marginTop:18,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(330px,1fr))",gap:14}}>{visibleGroups.filter(group=>!workspaceRepair||!group.repairs.some(repair=>sameUnit(repair,workspaceRepair))).map(group=>{const runningRepair=group.repairs.find(repair=>data?.activeTimer?.repairId===repair.id)??null,totalHours=group.repairs.reduce((sum,repair)=>sum+repair.laborHours,0),firstWorkable=group.repairs.find(repair=>repair.technicianId===data?.user.technicianId||repair.technicianId===null)??group.repairs[0];return <article key={group.key} style={{...unitCard,border:runningRepair?"2px solid #f47b20":"1px solid #dce2e7"}}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"baseline"}}><a href={unitInfoHref(group.unit)} target="_blank" rel="noreferrer" style={unitCardLink}>UNIT {group.unit||"—"} ↗</a><span style={{fontSize:12,fontWeight:900,color:"#5d6975"}}>{group.repairs.length} OPEN</span></div><div style={{marginTop:5,color:"#667482",fontSize:12}}>{totalHours.toFixed(2)} total labor hours</div><div style={{marginTop:12,display:"grid",gap:7}}>{group.repairs.map(repair=><div key={repair.id} style={{display:"flex",justifyContent:"space-between",gap:10,padding:"9px 0",borderTop:"1px solid #edf0f2"}}><span style={{fontWeight:800}}>{repair.issue}{repair.handoffNote&&<span style={{...handoffBadge,marginLeft:7}}>SHIFT HANDOFF</span>}</span><span style={lineState(repair,data?.activeTimer?.repairId)==="WAITING ON PART"?waitingBadge:lineState(repair,data?.activeTimer?.repairId)==="WORKING NOW"?workingBadge:openBadge}>{lineState(repair,data?.activeTimer?.repairId)}</span></div>)}</div>{runningRepair?<button onClick={()=>setSelectedId(runningRepair.id)} style={unitButton}>OPEN UNIT WORKSPACE</button>:!data?.activeTimer&&firstWorkable&&data?.user.technicianId?<button disabled={busy} onClick={()=>void startUnit(firstWorkable)} style={startUnitWide}>START WORKING ON UNIT</button>:<button onClick={()=>setSelectedId(firstWorkable?.id??null)} style={unitButton}>VIEW UNIT</button>}</article>})}{data&&!visibleGroups.length&&<div style={emptyState}>No units in this view.</div>}</section>}
  </main>
}
...
const unitInfoLink={color:"#0d1b2b",textDecoration:"underline",textDecorationThickness:2,textUnderlineOffset:4,fontWeight:950} as const;
const unitCardLink={fontSize:18,color:"#0d1b2b",fontWeight:950,textDecoration:"underline",textUnderlineOffset:3} as const;
const eyebrow={margin:0,color:"#f47b20",fontSize:11,fontWeight:900,letterSpacing:".14em"} as const;
const unitSessionActions={display:"flex",gap:8,alignItems:"center",flexWrap:"wrap" as const,justifyContent:"flex-end"} as const;
...
const handoffButton={border:0,borderRadius:10,padding:"12px 15px",background:"#176fe6",color:"white",fontWeight:950,cursor:"pointer"} as const;
const handoffPanel={marginTop:16,padding:16,border:"2px solid #75a6df",borderRadius:12,background:"#f2f7fd",display:"grid",gap:12} as const;
const handoffLabel={display:"grid",gap:5,fontSize:12,fontWeight:900,color:"#425565"} as const;
const handoffDivider={fontSize:11,fontWeight:950,letterSpacing:".08em",color:"#2e628e",textAlign:"center" as const} as const;
const doneForNowButton={border:"1px solid #7b8a96",borderRadius:9,padding:"12px 14px",background:"white",color:"#253440",fontWeight:950,cursor:"pointer"} as const;
const confirmHandoffButton={border:0,borderRadius:9,padding:"11px 15px",background:"#176fe6",color:"white",fontWeight:950,cursor:"pointer"} as const;
const handoffBadge={display:"inline-flex",padding:"4px 7px",borderRadius:999,background:"#dcecff",color:"#155aab",fontWeight:950,fontSize:10,whiteSpace:"nowrap" as const,verticalAlign:"middle"} as const;
const handoffNotice={marginTop:14,padding:12,border:"1px solid #75a6df",borderRadius:10,background:"#f2f7fd",color:"#24445f",fontSize:13} as const;
...
const outcomeBase={border:0,borderRadius:12,padding:"16px",fontWeight:900,fontSize:16,cursor:"pointer",textAlign:"left" as const,display:"grid",gap:4} as const;
const outcomeHint={display:"block",fontSize:13,fontWeight:700,lineHeight:1.35} as const;
