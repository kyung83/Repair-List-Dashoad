...
export default function InspectionFinal({repairId,canWork,manualMileage,onReady}:Props){
 ...
 async function finish(){
  ...
  try{
   const body:Record<string,unknown>={action:'markReady',repairId};
   if(data.mileageEntryAllowed&&manualMileage.trim())body.mileage=manualMileage;
   ...
   setData(p);onReady(p);setMessage(p.mileageSource==='Unavailable'?"Signed inspection finished and locked. Mileage was unavailable, but the maintenance record was saved.":p.mileageSource==='Verified Manual'?"Signed inspection finished and locked with verified manual mileage.":"Signed inspection finished and locked. Print the truck copy below.");
