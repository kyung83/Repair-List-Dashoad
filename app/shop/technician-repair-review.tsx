...
type ReviewNote={id:number;detail:string;technician:string;createdAt:string};
...
type ReviewData={ok?:boolean;error?:string;repair:{id:string;unit:string;title:string;status:string};notes:ReviewNote[];parts:ReviewPart[];labor:ReviewLabor[];requests:ReviewRequest[];updatedAt:string};
...
export default function TechnicianRepairReview({repairId,canWork,checklist}:Props){
  const[mounted,setMounted]=useState(false),[data,setData]=useState<ReviewData|null>(null),[message,setMessage]=useState(""),[removingPartId,setRemovingPartId]=useState<number|null>(null),[loading,setLoading]=useState(false);
  ...
  async function removePart(part:ReviewPart){
    if(!canWork||removingPartId!==null)return;
  ...
  const panel=<section style={outer}>
    <div style={card}>
      <div style={header}>
        <div><p style={eyebrow}>FINAL CHECK</p><h2 style={title}>Review Before Finishing</h2><div style={sub}>{data?`Unit ${data.repair.unit||"—"} · ${data.repair.title} · ${data.repair.status}`:"Everything recorded on this repair stays visible here."}</div></div>
      ...
      <div style={grid}>
        <div style={sectionCard}>
          ...
          {data?.notes.length?<div style={list}>{data.notes.map(item=><div key={item.id} style={row}><div style={{whiteSpace:"pre-wrap"}}>{item.detail}</div><div style={meta}>{item.technician} · {dateTime(item.createdAt)}</div></div>)}</div>:<div style={empty}>No repair notes saved yet.</div>}
        </div>
        ...
        <div style={sectionCard}>
          ...
          {data?.parts.length?<div style={list}>{data.parts.map(part=><div key={part.partId} style={partRow}><div><strong>{part.partNumber}</strong><div style={description}>{part.description}</div>{part.lastAppliedAt&&<div style={meta}>Applied {dateTime(part.lastAppliedAt)}</div>}</div><div style={partActions}><strong style={partQty}>× {qty(part.quantity)}</strong><button type="button" aria-label={`Remove ${part.partNumber}`} title="Remove mistaken part" onClick={()=>void removePart(part)} style={removeButton} disabled={!canWork||removingPartId!==null}>{removingPartId===part.partId?"…":"✕"}</button></div></div>)}</div>:<div style={empty}>No parts applied to this repair.</div>}
