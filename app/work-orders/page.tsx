          ...
          return <Fragment key={item.id}>
            ...
            {open&&<tr style={{borderTop:"1px solid #e7ebee",background:"#fafbfc"}}><td colSpan={11} style={{padding:14}}>
              ...
              <div style={{marginTop:12,display:"grid",gridTemplateColumns:"minmax(460px,2fr) minmax(280px,1fr)",gap:12}}>
                <div style={panelStyle}><div style={subheadStyle}>Parts applied</div>{item.usedParts.length?item.usedParts.map((part,index)=><div key={`${part.repairId}-${part.partId}-${index}`} style={{display:"grid",gridTemplateColumns:"105px 1fr 60px 95px 100px",gap:8,padding:"7px 9px",borderTop:"1px solid #edf0f2",fontSize:11}}><strong>{part.partNumber}</strong><span>{part.description}<small style={{display:"block",color:"#7a858d"}}>{part.repairIssue}</small></span><span>× {part.quantity}</span><span>{money(part.unitCost)} ea.</span><strong>{money(part.lineCost)}</strong></div>):<div style={emptyStyle}>No parts applied.</div>}</div>
                <div style={panelStyle}><div style={subheadStyle}>Cost summary</div><Cost label="Labor" value={item.laborCost}/><Cost label="Parts" value={item.partCost}/><Cost label="Outside" value={item.outsideCost}/><Cost label="TOTAL" value={item.totalCost} strong/></div>
              </div>
