...
function matchEquipment(lines:string[],text:string,equipment:Equipment[]){
  ...
  const vinCandidates=text.toUpperCase().match(/\b[A-HJ-NPR-Z0-9]{17}\b/g)||[];
...
function detectInvoiceNumber(lines:string[]){
  const labels=[
    /\bINVOICE\s*(?:NUMBER|NO\.?|#)\b/i,/\bINV\s*(?:NUMBER|NO\.?|#)\b/i,
    /\bREPAIR\s*ORDER\s*(?:NUMBER|NO\.?|#)?\b/i,/\bR\.?O\.?\s*(?:NUMBER|NO\.?|#)\b/i,
    /\bWORK\s*ORDER\s*(?:NUMBER|NO\.?|#)?\b/i,/\bW\.?O\.?\s*(?:NUMBER|NO\.?|#)\b/i,
    /\bDOCUMENT\s*(?:NUMBER|NO\.?|#)\b/i,
  ];
  for(const label of labels){
    for(let i=0;i<lines.length;i++){
      if(!label.test(lines[i]))continue;
      const same=cleanReference(valueAfterLabel(lines[i],label));if(same)return same;
      const next=cleanReference(lines[i+1]||"");if(next)return next;
    }
  }
  for(const line of lines.slice(0,40)){
    const match=line.match(/\b(?:INVOICE|INV)\s*[:#-]\s*([A-Z0-9][A-Z0-9./_-]{2,30})\b/i);
    const value=match?.[1]||"";if(value&&!/DATE|TOTAL/i.test(value))return value;
  }
  return"";
}
...
function detectDate(lines:string[]){
  ...
  for(const label of labels){
    for(let i=0;i<Math.min(lines.length,80);i++){
      if(!label.test(lines[i]))continue;
      const same=dateTokens(valueAfterLabel(lines[i],label));if(same[0])return same[0];
      const next=dateTokens(lines[i+1]||"");if(next[0])return next[0];
    }
  }
  return"";
}
...
function detectMileage(lines:string[]){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    const values=(after.match(/\b\d[\d,]{2,9}\b/g)||[]).map(value=>Number(value.replace(/,/g,""))).filter(value=>Number.isInteger(value)&&value>=0&&value<=10_000_000);
    if(!values.length)continue;
    if(/\bOUT\b/i.test(source)||values.length>1)return String(Math.max(...values));
...
function detectTotal(lines:string[]){
  ...
  for(const label of labels){
    for(let i=lines.length-1;i>=0;i--){
      ...
      const values=amountValues(valueAfterLabel(lines[i],label));if(values.length)return values.at(-1)!.toFixed(2);
      const next=amountValues(lines[i+1]||"");if(next.length)return next.at(-1)!.toFixed(2);
    }
  }
  return"";
}
...
function detectVendor(lines:string[]){
  ...
  for(let i=0;i<Math.min(lines.length,22);i++){
    ...
    const letters=(line.match(/[A-Z]/gi)||[]).length;const digits=(line.match(/\d/g)||[]).length;if(letters<3||digits>letters)continue;
    let score=30-i;
    ...
    if(line===line.toUpperCase())score+=8;if(line.length>=8&&line.length<=55)score+=8;
...
function detectServiceLines(lines:string[]){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    for(let j=i+1;j<Math.min(lines.length,i+35);j++){
      const line=lines[j];if(j>i+1&&stop.test(line))break;
      if(metadata.test(line)||line.length<4||line.length>220||!/[A-Za-z]{3}/.test(line))continue;
      const moneyCount=amountValues(line).length;const letters=(line.match(/[A-Za-z]/g)||[]).length;if(moneyCount>=2&&letters<16)continue;
      candidates.push(line);
    }
    if(candidates.length)break;
  }
  for(const line of lines){if(line.length>=4&&line.length<=220&&!metadata.test(line)&&keyword.test(line))candidates.push(line);}
...
function detectDocument(text:string,equipment:Equipment[]):Detected{
  ...
  return {
    equipment:matchEquipment(lines,text,equipment),invoiceNumber:detectInvoiceNumber(lines)||undefined,
    invoiceDate:detectDate(lines)||undefined,mileage:detectMileage(lines)||undefined,totalAmount:detectTotal(lines)||undefined,
    vendorName:detectVendor(lines)||undefined,serviceSummary:detectServiceLines(lines)||undefined,
  };
}

function improveCanvasForOcr(canvas:HTMLCanvasElement){
  const context=canvas.getContext('2d',{willReadFrequently:true});if(!context)return canvas;
  try{
    const image=context.getImageData(0,0,canvas.width,canvas.height);const data=image.data;
    for(let i=0;i<data.length;i+=4){const gray=.299*data[i]+.587*data[i+1]+.114*data[i+2];const adjusted=Math.max(0,Math.min(255,128+(gray-128)*1.55));data[i]=adjusted;data[i+1]=adjusted;data[i+2]=adjusted;}
...
async function imageFileToCanvas(file:File){
  ...
  try{
    const longest=Math.max(bitmap.width,bitmap.height);const scale=Math.min(2.4,Math.max(1,2600/Math.max(1,longest)));
    const canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));
    const context=canvas.getContext('2d');if(!context)throw new Error('The browser could not prepare this scan for OCR.');
    context.fillStyle='#fff';context.fillRect(0,0,canvas.width,canvas.height);context.drawImage(bitmap,0,0,canvas.width,canvas.height);return improveCanvasForOcr(canvas);
...
function loadScript(id:string,src:string){
  return new Promise<void>((resolve,reject)=>{
    ...
    if(!script){script=document.createElement('script');script.id=id;script.src=src;script.async=true;script.crossOrigin='anonymous';document.head.appendChild(script);}
    ...
    const loaded=()=>finish();const failed=()=>finish(new Error('Scan reader could not load. Please try Read Scan Again.'));
    const timer=window.setTimeout(()=>finish(new Error('Scan reader timed out while loading. Please try Read Scan Again.')),20000);
    target.addEventListener('load',loaded,{once:true});target.addEventListener('error',failed,{once:true});
  });
}

export default function CameraOutsideWorkIntake(){
  ...
  async function load(){
    const response=await fetch('/api/outside-work',{cache:'no-store'});const payload=await response.json() as Payload;
    ...
    if(requested&&!form.equipmentId){const found=payload.equipment.find(row=>normalizeUnit(row.unit)===normalizeUnit(requested));if(found){setUnitInput(found.unit);setForm(current=>({...current,equipmentId:String(found.id)}));}}
  }
  ...
  useEffect(()=>{
    ...
    videoRef.current.srcObject=streamRef.current;void videoRef.current.play().catch(()=>setMessage('Camera opened, but video preview could not start. Close Camera and try again.'));
  },[cameraOpen]);
  ...
  function setChosenUnit(value:string){
    setUnitInput(value);const normalized=normalizeUnit(value);const matches=data.equipment.filter(row=>normalizeUnit(row.unit)===normalized);
    setForm(current=>({...current,equipmentId:matches.length===1?String(matches[0].id):""}));
  }

  function replaceScan(next:File|null,{resetForm=false}:{resetForm?:boolean}={}){
    if(previewUrl)URL.revokeObjectURL(previewUrl);
    setScan(next);setPreviewUrl(next?URL.createObjectURL(next):"");setRawText("");setProgress("");
    if(resetForm){
      const requested=new URLSearchParams(window.location.search).get('unit')||'';const preselected=data.equipment.find(row=>normalizeUnit(row.unit)===normalizeUnit(requested));
  ...
  function applyRules(text=rawText){
    ...
    setMessage(found.length?`Scan read. Auto-filled: ${found.join(', ')}. Verify every field against the photo before saving.`:'Scan read, but it did not find enough reliable labeled information. Correct the fields manually before saving.');
  }
  ...
  async function createOcrWorker(){
    ...
    await worker.setParameters?.({preserve_interword_spaces:'1',user_defined_dpi:'300'});return worker;
  }

  async function readScan(target:File|null=scan){
    if(!target){setMessage('Take a camera scan first.');return;}
    if(reading||busy)return;
    setReading(true);setMessage('Reading camera scan...');setProgress('Starting scan reader...');
    try{
      const worker=await createOcrWorker();let canvas:HTMLCanvasElement|null=null;
      try{setProgress('Preparing scan for OCR...');canvas=await imageFileToCanvas(target);setProgress('Reading scan...');const text=(await worker.recognize(canvas)).data.text||"";const trimmed=text.trim();setRawText(trimmed);if(!trimmed)setMessage('No readable text was found. Retake the scan with the invoice flat, bright and filling the frame.');else applyRules(trimmed);}
      finally{await worker.terminate();if(canvas){canvas.width=1;canvas.height=1;}}
    }catch(error){setMessage(`${error instanceof Error?error.message:'Scan could not be read.'} Retake the photo or enter the fields manually after a scan is captured.`);}
    finally{setReading(false);setProgress('');}
  }

  async function startCamera(){
    if(reading||busy||cameraStarting)return;
    if(!navigator.mediaDevices?.getUserMedia){setMessage('This browser cannot open the camera. Camera access is required because file attachments are disabled.');return;}
    stopCamera();setCameraStarting(true);setMessage('Opening camera...');
    try{
      ...
      streamRef.current=stream;setCameraOpen(true);setMessage('Camera ready. Fill the frame with the invoice, keep it flat, and avoid glare.');
    }catch(error){
      const detail=error instanceof Error&&error.name==='NotAllowedError'?'Camera permission was denied. Allow camera access in the browser and try again.':'The camera could not be opened. Check camera permission and try again.';
      setMessage(detail);
    }finally{setCameraStarting(false);}
  }

  async function captureScan(){
    const video=videoRef.current;if(!video||video.readyState<2||!video.videoWidth||!video.videoHeight){setMessage('Camera is not ready yet. Wait a moment and press Capture Invoice again.');return;}
    const canvas=document.createElement('canvas');canvas.width=video.videoWidth;canvas.height=video.videoHeight;const context=canvas.getContext('2d');if(!context){setMessage('The browser could not capture the camera image.');return;}
    context.drawImage(video,0,0,canvas.width,canvas.height);
    const blob=await new Promise<Blob|null>(resolve=>canvas.toBlob(resolve,'image/jpeg',.94));canvas.width=1;canvas.height=1;
    if(!blob){setMessage('The camera image could not be saved. Try again.');return;}
    const stamp=new Date().toISOString().replace(/[:.]/g,'-');const captured=new File([blob],`outside-work-scan-${stamp}.jpg`,{type:'image/jpeg',lastModified:Date.now()});
    stopCamera();replaceScan(captured);setMessage('Scan captured. Reading invoice...');await readScan(captured);
  }

  async function submit(event:FormEvent){
    event.preventDefault();setMessage('');
    if(!scan){setMessage('Take a camera scan of the outside-work invoice first.');return;}
    ...
    try{
      ...
      setMessage(`Recorded ${result.repairId||'outside work'} for unit ${result.unit||selectedEquipment?.unit||''}. The camera scan is stored with the repair history.`);replaceScan(null,{resetForm:true});await load();
  ...
  return <main style={page}><div style={shell}>
    ...
    <header style={{marginTop:24}}><p style={eyebrow}>SHOP · OUTSIDE WORK</p><h1 style={title}>Scan outside work into unit history</h1><p style={subtitle}>Use the camera to scan the vendor invoice. There is no file-upload or attachment picker. The scan is read with fixed OCR rules, then you verify the unit, work and cost before anything is recorded.</p></header>
    {message&&<div style={notice}>{message}</div>}

    <section style={{...panel,marginTop:18}}>
      <div style={sectionHead}><div><h2 style={h2}>1. Scan vendor invoice</h2><p style={copy}>Camera only. Put the full invoice in the frame, keep it flat, and use good light with as little glare as possible.</p></div></div>
      <div style={{display:'grid',gridTemplateColumns:'minmax(280px,.7fr) minmax(340px,1.3fr)',gap:16,alignItems:'start'}}>
        <div style={{display:'grid',gap:10}}>
          {!cameraOpen&&<button type="button" style={{...primary,opacity:(reading||busy||cameraStarting)?.6:1}} disabled={reading||busy||cameraStarting} onClick={()=>void startCamera()}>{cameraStarting?'OPENING CAMERA...':scan?'RETAKE SCAN':'OPEN CAMERA'}</button>}
          {cameraOpen&&<><button type="button" style={primary} disabled={reading||busy} onClick={()=>void captureScan()}>CAPTURE INVOICE</button><button type="button" style={button} onClick={stopCamera}>CLOSE CAMERA</button></>}
          {scan&&!cameraOpen&&<button type="button" style={{...button,opacity:reading?.6:1}} disabled={reading||busy} onClick={()=>void readScan()}>{reading?'READING SCAN...':'READ SCAN AGAIN'}</button>}
          {progress&&<div style={progressBox}>{progress}</div>}
          <div style={small}><strong>No attachment button.</strong> The only way to add the source document here is to take the camera scan. The captured image is kept as the original service evidence after you save the repair.</div>
        </div>
        <div style={previewBox}>
          {cameraOpen?<video ref={videoRef} playsInline muted style={videoStyle}/>:previewUrl?<img src={previewUrl} alt="Captured outside work invoice" style={{maxWidth:'100%',maxHeight:520,objectFit:'contain'}}/>:<div style={empty}>Open the camera to scan the invoice.</div>}
        </div>
      </div>
      <details style={{marginTop:14}} open={Boolean(rawText)}><summary style={{cursor:'pointer',fontWeight:850}}>Scan text / OCR result</summary><p style={small}>If OCR misreads a vendor format, correct the extracted text here and press Apply Rules to Text. This does not change the original camera scan.</p><textarea value={rawText} onChange={event=>setRawText(event.target.value)} placeholder="OCR text will appear here after the camera scan is read." style={{...input,minHeight:180,fontFamily:'ui-monospace,SFMono-Regular,Menlo,monospace'}}/><button type="button" style={{...button,marginTop:8}} onClick={()=>applyRules()}>APPLY RULES TO TEXT</button></details>
    </section>
    ...
    <form onSubmit={submit} style={{...panel,marginTop:16}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap',marginTop:14}}><span style={small}>Creates one completed <strong>Outside Work</strong> repair and keeps the camera scan as the source evidence.</span><button type="submit" style={{...primary,minHeight:44,opacity:busy?.6:1}} disabled={busy}>{busy?'SAVING...':'CREATE OUTSIDE REPAIR RECORD'}</button></div>
    </form>

    <section style={{...panel,marginTop:16}}>
      <div style={sectionHead}><div><h2 style={h2}>Recorded outside work</h2><p style={copy}>The newest 100 vendor records. Open the original scan at any time or jump to the unit.</p></div><strong>{data.records.length} shown</strong></div>
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',minWidth:1040}}><thead><tr>{['Unit','Vendor / invoice','Service date','Mileage','Total','Work performed','Recorded','Original scan'].map(label=><th key={label} style={th}>{label}</th>)}</tr></thead><tbody>
        {data.records.map(row=><tr key={row.id} style={{borderTop:'1px solid #edf0f2'}}><td style={td}><a href={`/unit?unit=${encodeURIComponent(row.unit)}`} style={link}><strong>{row.unit}</strong></a><div style={mini}>{row.repairId}</div></td><td style={td}><strong>{row.vendorName||'Vendor not entered'}</strong><div style={mini}>{row.invoiceNumber||'No invoice number'}</div></td><td style={td}>{row.invoiceDate||'—'}</td><td style={td}>{row.mileage==null?'—':`${row.mileage.toLocaleString()} mi`}</td><td style={td}><strong>{money(row.totalAmount)}</strong></td><td style={{...td,maxWidth:340,whiteSpace:'pre-line'}}>{row.serviceSummary}</td><td style={td}>{when(row.createdAt)}<div style={mini}>{row.uploadedBy||'Manager'}</div></td><td style={td}><a href={row.originalUrl} target="_blank" rel="noreferrer" style={actionLink}>Open scan</a></td></tr>)}
        {!data.records.length&&<tr><td colSpan={8} style={{...td,textAlign:'center',padding:28,color:'#71808c'}}>No outside-work records have been recorded yet.</td></tr>}
      </tbody></table></div>
    </section>
  </div></main>;
}
...
const previewBox:CSSProperties={minHeight:300,display:'grid',placeItems:'center',border:'1px solid #dbe2e7',borderRadius:10,background:'#111923',overflow:'hidden'};
const videoStyle:CSSProperties={width:'100%',maxHeight:520,objectFit:'contain',background:'#111923'};
const empty:CSSProperties={padding:24,color:'#c3ccd4',textAlign:'center'};
...
const button:CSSProperties={border:'1px solid #bcc9d2',borderRadius:8,padding:'10px 12px',background:'white',color:'#17324a',fontWeight:850,cursor:'pointer'};
