...
type Payload={forms:PmForm[];updatedAt:string;error?:string};
...
export default function PmInspectionsPage(){
  ...
  async function load(){
    setLoading(true);
    try{
      const response=await fetch("/api/pm-inspections",{cache:"no-store"});
      const payload=await response.json() as Payload;
      if(!response.ok)throw new Error(payload.error||"PM forms could not be loaded.");
      setForms(payload.forms);
      setMessage("");
    }catch(error){
      setMessage(error instanceof Error?error.message:"PM forms could not be loaded.");
  ...
  return <main style={{minHeight:"100vh",background:"#f3f5f7",color:"#172033",padding:"34px 34px 110px"}}>
    ...
    <header style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",gap:18,flexWrap:"wrap"}}>
      <div>
        <p style={{margin:0,color:"#f47b20",fontWeight:900,letterSpacing:".14em",fontSize:12}}>PREVENTIVE MAINTENANCE RECORDS</p>
        <h1 style={{margin:"7px 0 4px",fontSize:34}}>Completed PM Forms</h1>
        <p style={{margin:0,maxWidth:850,color:"#64748b",lineHeight:1.5}}>
          Completed PM inspection sheets are stored here for printing and PDF saving. If a paper copy is lost, search the unit and open the same signed PM sheet again.
        </p>
      </div>
    ...
    <section style={searchCard}>
      <label style={{display:"grid",gap:5,fontWeight:800,maxWidth:620}}>
        Find a PM form
        <input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Unit, VIN, plate, mechanic, report number…" style={inputStyle}/>
      </label>
      <div style={{color:"#64748b",fontSize:12}}>{visible.length} completed form{visible.length===1?"":"s"}</div>
    </section>

    <section style={{marginTop:16,display:"grid",gap:10}}>
      {visible.map(form=><article key={form.runId} style={formCard}>
        <div style={{minWidth:0}}>
          ...
          <div style={{marginTop:5,color:"#52616c",lineHeight:1.55}}>
            ...
            {form.inspector?` · Mechanic: ${form.inspector}`:""}
      ...
      {!loading&&!visible.length&&<div style={emptyStyle}>{forms.length?"No PM forms match that search.":"No completed PM checklist forms are stored yet."}</div>}
    </section>
  </main>;
}
