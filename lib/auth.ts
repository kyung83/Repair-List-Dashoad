...
function secureEqual(left: Uint8Array, right: Uint8Array) {
  ...
  return lengthsMatch ? subtle.timingSafeEqual(left, right) : !subtle.timingSafeEqual(left, left);
}
...
async function deriveLegacyPbkdf2(password: string, salt: Uint8Array, iterations: number) {
  const key = await globalThis.crypto.subtle.importKey('raw', encoder.encode(password), { name: 'PBKDF2' }, false, ['deriveBits']);
...
function deriveScrypt(password: string, salt: Uint8Array) {
  return new Uint8Array(scryptSync(Buffer.from(password, 'utf8'), Buffer.from(salt), 32, { N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P, maxmem: SCRYPT_MAXMEM }));
}
function normalizePasswordAlgorithm(value: unknown): PasswordAlgorithm { return value === 'scrypt-v1' ? 'scrypt-v1' : 'pbkdf2-sha256'; }
...
export async function verifyPassword(password: string, hash: string, salt: string, iterations: number, algorithmValue: unknown = 'pbkdf2-sha256') {
  try {
    const algorithm = normalizePasswordAlgorithm(algorithmValue);
    const saltBytes = base64UrlToBytes(salt);
    const actual = algorithm === 'scrypt-v1' ? deriveScrypt(password, saltBytes) : await deriveLegacyPbkdf2(password, saltBytes, iterations || LEGACY_PASSWORD_ITERATIONS);
...
function cookieValue(cookieHeader: string | null, name: string) {
  ...
  for (const part of cookieHeader.split(';')) { const [key, ...rest] = part.trim().split('='); if (key === name) return rest.join('='); }
  return '';
}
export function sessionTokenFromRequest(request: Request) { return cookieValue(request.headers.get('cookie'), SESSION_COOKIE); }
...
export async function createSession(db: D1Database, userId: number) {
  ...
  await db.prepare(`INSERT INTO app_sessions (token_hash, user_id, expires_at) VALUES (?, ?, datetime('now', ?))`).bind(tokenHash, userId, `+${SESSION_HOURS} hours`).run();
  return token;
}
export async function deleteSession(db: D1Database, request: Request) {
  const token = sessionTokenFromRequest(request); if (!token) return;
  const tokenHash = bytesToBase64Url(await sha256(token));
...
async function loginBlocked(db:D1Database, attemptKey:string) {
  const row=await db.prepare(`SELECT failures FROM app_login_attempts WHERE attempt_key=? AND window_started_at>=datetime('now',?)`).bind(attemptKey,`-${LOGIN_WINDOW_MINUTES} minutes`).first<{failures:number}>();
  return Number(row?.failures??0)>=MAX_LOGIN_FAILURES;
}
async function recordLoginFailure(db:D1Database,attemptKey:string){await db.prepare(`INSERT INTO app_login_attempts(attempt_key,failures,window_started_at,updated_at) VALUES(?,1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP) ON CONFLICT(attempt_key) DO UPDATE SET failures=CASE WHEN app_login_attempts.window_started_at<datetime('now',?) THEN 1 ELSE app_login_attempts.failures+1 END,window_started_at=CASE WHEN app_login_attempts.window_started_at<datetime('now',?) THEN CURRENT_TIMESTAMP ELSE app_login_attempts.window_started_at END,updated_at=CURRENT_TIMESTAMP`).bind(attemptKey,`-${LOGIN_WINDOW_MINUTES} minutes`,`-${LOGIN_WINDOW_MINUTES} minutes`).run();}
