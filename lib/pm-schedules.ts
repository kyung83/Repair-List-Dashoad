...
function equipmentIds(value: unknown) {
  ...
  if (ids.length > 1000) throw new Error('Too many vehicles selected at once.');
  return ids;
}
  ...
  const [profilesResult, equipmentResult] = await Promise.all([
    ...
    db.prepare(`
      SELECT e.id, e.unit, e.category, e.equipment_type, e.current_mileage, e.mileage_updated_at,
             ...
             ps.pm_type, ps.last_mileage, ps.service_date, ps.annual_date
      FROM equipment e
...
export async function applyPmSchedule(db: D1Database, body: Record<string, unknown>) {
  ...
  const lastMileage = optionalInteger(body.lastMileage, 'Last PM mileage', { allowZero: true });
  const lastServiceDate = dateOnly(body.lastServiceDate, 'Last PM/service date');
  const lastAnnualDate = dateOnly(body.lastAnnualDate, 'Last annual date');
  ...
  for (const id of ids) {
    ...
    `).bind(id, profileId, mileageInterval, timeIntervalDays, annualRequired));
    ...
    statements.push(db.prepare(`
      ...
      ON CONFLICT(equipment_id) DO UPDATE SET
        ...
        updated_at = CURRENT_TIMESTAMP
    `).bind(id, sequence[0], lastMileage, lastServiceDate, lastAnnualDate));
  }
...
export async function setPmEquipmentCategory(db: D1Database, body: Record<string, unknown>) {
  ...
  if (!(PM_CATEGORIES as readonly string[]).includes(category) || category === 'Uncategorized') {
    throw new Error('Choose a valid PM category.');
  }
  if (category === 'Trailers') {
    const trailer = await db.prepare(`SELECT equipment_type FROM equipment WHERE id = ?`).bind(equipmentId)
      .first<{ equipment_type: string }>();
    if (!trailer || trailer.equipment_type !== 'trailer') throw new Error('Only trailers can use the Trailers category.');
  }
  await db.prepare(`
...
export async function updatePmMileage(db: D1Database, body: Record<string, unknown>) {
  ...
  const result = await db.prepare(`
    ...
    WHERE id = ? AND geotab_device_id IS NULL
...
export async function recordPmCompletion(db: D1Database, body: Record<string, unknown>) {
  ...
  const mileage = suppliedMileage ?? row.current_mileage;
