...
export async function diagnoseGeotabConnection(env: DiagnosticEnv): Promise<GeotabDiagnosticResult> {
  ...
  const probes: Array<[string, () => Promise<unknown>]> = [
    ...
    ['dvirFeed', () => call(auth, 'GetFeed', { typeName: 'DVIRLog', resultsLimit: 1, search: { fromDate } })],
  ];
  ...
  return {
    ...
    checks,
    ...(ok ? {} : { error: 'The service account authenticated, but one or more API permissions failed.' }),
  };
}
