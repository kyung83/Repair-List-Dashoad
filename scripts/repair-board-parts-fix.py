...
''',
    'inventory load and editor reset',
)
...
''',
  ...
  const q = query.trim().toLowerCase();
''',
    'repair save and inventory attach action',
)

pattern = re.compile(
    r'<label className="wide">\s*Parts needed\s*<input\s*value=\{editingRepair\.parts\}\s*onChange=\{\(event\) => setEditingRepair\(\{ \.\.\.editingRepair, parts: event\.target\.value \}\)\}\s*/>\s*</label>',
    re.MULTILINE,
)
replacement = '\n'.join([
...
if count != 1:
    raise SystemExit(f'repair modal parts picker: expected exactly one match, found {count}')
